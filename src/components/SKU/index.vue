...
export default {
  ...
  setup(props, { emit }) {
    ...
    watchEffect(() => {
      // 得到所有字典集合
      pathMap = getPathMap(props.products.skus)
      // 组件初始化的时候更新禁用状态
      initDisabledStatus(props.products.specs, pathMap)
    })
    ...
    const clickSpecs = (item, val) => {
      ...
      // 点击之后再次更新选中状态
      updateDisabledStatus(props.products.specs, pathMap)
      // 把选择的sku信息传出去给父组件
      // 触发change事件将sku数据传递出去
      const selectedArr = getSelectedArr(props.products.specs).filter(value => value)
      // 如果选中得规格数量和传入得规格总数相等则传出完整信息(都选择了)
      // 否则传出空对象
      if (selectedArr.length === props.products.specs.length) {
        // 从路径字典中得到skuId
        const skuId = pathMap[selectedArr.join(spliter)][0]
        const sku = props.products.skus.find(sku => sku.id === skuId)
        // 传递数据给父组件
        emit('change', {
          skuId: sku.id,
          price: sku.price,
